...
function parseMultiPolygon(wkt: string): GeoJSON.MultiPolygon | null {
  ...
  for (let i = 0; i < polygonStrings.length; i++) {
    ...
    const coordPairs = polygonString.split(',').map((pair) => {
      ...
      if (coords.length !== 2) {
        console.warn('Invalid coordinate pair:', cleanPair, 'split into:', coords);
        return null;
      }
      ...
      if (isNaN(lon) || isNaN(lat)) {
        console.warn('Invalid coordinate values:', coords, 'from pair:', cleanPair);
        return null;
      }
    ...
    // Validate coordinates
    if (coordPairs.length < 3) {
      console.warn('Invalid polygon in MULTIPOLYGON: need at least 3 points, got', coordPairs.length);
      continue;
    }
 ...
 */
export function normalizeGeometry(geometry: string | GeoJSON.Polygon | GeoJSON.MultiPolygon | { wkt: string } | null): GeoJSON.Polygon | GeoJSON.MultiPolygon | null {
  if (!geometry) return null;

  // If it's already GeoJSON
  if (typeof geometry === 'object' && 'type' in geometry && 'coordinates' in geometry) {
