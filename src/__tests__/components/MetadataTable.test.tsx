...
describe('MetadataTable Component', () => {
  describe('Rendering with All Fields', () => {
    it('renders the table header with title and full details link', () => {
      ...
      expect(screen.getByRole('link', { name: 'Full Details ↓' })).toHaveAttribute('href', '#full-details');
    });
    ...
    it('renders description correctly', () => {
      ...
      expect(screen.getByText('Description')).toBeInTheDocument();
      expect(screen.getByText('A historical paper map from MIT collections')).toBeInTheDocument();
    });
    ...
    it('renders spatial coverage as array', () => {
      ...
      expect(screen.getByText('Places')).toBeInTheDocument();
      expect(screen.getByText('Massachusetts, United States')).toBeInTheDocument();
    });
    ...
    it('renders combined row with temporal, date, and language', () => {
      ...
      const temporalSection = screen.getByText('Temporal Coverage').closest('td');
    ...
    it('renders subjects as comma-separated list', () => {
      ...
      expect(screen.getByText('Subject')).toBeInTheDocument();
      expect(screen.getByText('Paper Maps, Historical Maps, Library Collections')).toBeInTheDocument();
    });
  ...
  describe('Conditional Rendering', () => {
    it('renders only available fields when some are missing', () => {
      ...
      // Should render these
      expect(screen.getByText('Description')).toBeInTheDocument();
      expect(screen.getByText('A point dataset from NYU with web services')).toBeInTheDocument();
      expect(screen.getByText('Temporal Coverage')).toBeInTheDocument();
  ...
  describe('Layout and Styling', () => {
    it('applies correct table structure', () => {
      const { container } = render(<MetadataTable data={realFixtureWithAllFields} />);
    ...
    it('applies correct cell styling', () => {
      ...
      const descriptionValue = screen.getByText('A historical paper map from MIT collections');
  ...
  describe('Accessibility', () => {
    ...
    it('has proper link accessibility', () => {
      ...
      const fullDetailsLink = screen.getByRole('link', { name: 'Full Details ↓' });
      expect(fullDetailsLink).toHaveAttribute('href', '#full-details');
      expect(fullDetailsLink).toHaveClass('text-sm', 'text-blue-600', 'hover:text-blue-800');
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('renders with different fixture data types', () => {
      ...
      expect(screen.getByText('Description')).toBeInTheDocument();
      expect(screen.getByText('A polygon dataset from Tufts with comprehensive metadata')).toBeInTheDocument();
      expect(screen.getByText('Places')).toBeInTheDocument();
      ...
      expect(screen.getByText('Subject')).toBeInTheDocument();
      expect(screen.getByText('Polygon Data, Grid Data, Administrative Boundaries')).toBeInTheDocument();
      expect(screen.getByText('Resource Type')).toBeInTheDocument();
    ...
    it('renders with restricted access fixture', () => {
      ...
      expect(screen.getByText('Description')).toBeInTheDocument();
      expect(screen.getByText('A restricted raster dataset from Stanford')).toBeInTheDocument();
      expect(screen.getByText('Places')).toBeInTheDocument();
    ...
    it('handles very long text content', () => {
      const dataWithLongText = {
        data: {
          attributes: {
            dct_description_sm: ['This is a very long description that might contain extensive information about the dataset and should still render correctly without breaking the layout or causing any display issues.'],
            dct_spatial_sm: ['Very Long Place Name That Might Contain Extensive Information About Geographic Location'],
            dc_subject_sm: ['Very Long Subject Name That Might Contain Additional Details', 'Another Very Long Subject Name'],
          },
        },
      };
