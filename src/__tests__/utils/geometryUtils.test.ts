  ...
  describe('wktToGeoJSON', () => {
    describe('Single Polygon Parsing', () => {
      ...
      it('automatically closes unclosed polygon', () => {
        const wkt = 'POLYGON((-96.796 48.756, -90.379 48.756, -90.379 43.429, -96.796 43.429))';
        const result = wktToGeoJSON(wkt);
      ...
      it('returns null for invalid polygon format', () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('WKT is not a POLYGON or MULTIPOLYGON:', wkt);
      ...
      it('returns null for polygon with insufficient points', () => {
        ...
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith('Invalid polygon: need at least 3 points');
        
        consoleSpy.mockRestore();
      });

      it('handles polygon with negative coordinates', () => {
        const wkt = 'POLYGON((-122.2 37.4, -122.1 37.4, -122.1 37.5, -122.2 37.5, -122.2 37.4))';
        const result = wktToGeoJSON(wkt);
    ...
    describe('MultiPolygon Parsing', () => {
      it('converts a simple WKT multipolygon to GeoJSON', () => {
        const wkt = 'MULTIPOLYGON(((-96.796 48.756, -90.379 48.756, -90.379 43.429, -96.796 43.429, -96.796 48.756)), ((-122.2 37.4, -122.1 37.4, -122.1 37.5, -122.2 37.5, -122.2 37.4)))';
        const result = wktToGeoJSON(wkt);
      ...
      it('handles case-insensitive multipolygon keyword', () => {
        const wkt = 'multipolygon(((-96.796 48.756, -90.379 48.756, -90.379 43.429, -96.796 43.429, -96.796 48.756)))';
        const result = wktToGeoJSON(wkt);
      ...
      it('returns null for invalid multipolygon format', () => {
        ...
        const wkt = 'INVALID(((-96.796 48.756, -90.379 48.756, -90.379 43.429, -96.796 43.429, -96.796 48.756)))';
        const result = wktToGeoJSON(wkt);
      ...
      it('skips invalid polygons in multipolygon and continues with valid ones', () => {
        ...
        const wkt = 'MULTIPOLYGON(((-96.796 48.756, -90.379 48.756)), ((-122.2 37.4, -122.1 37.4, -122.1 37.5, -122.2 37.5, -122.2 37.4)))';
        const result = wktToGeoJSON(wkt);
        
        expect(result).toBeDefined();
        expect(result?.type).toBe('MultiPolygon');
        expect(Array.isArray(result?.coordinates)).toBe(true);
        expect(consoleSpy).toHaveBeenCalledWith('Invalid polygon in MULTIPOLYGON: need at least 3 points, got', 2);
      ...
      it('returns null when no valid polygons found in multipolygon', () => {
        ...
        const wkt = 'MULTIPOLYGON(((-96.796 48.756, -90.379 48.756)), ((-122.2 37.4, -122.1 37.4)))';
        const result = wktToGeoJSON(wkt);
        
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith('No valid polygons found in MULTIPOLYGON');
      ...
      it('handles invalid coordinate pairs in multipolygon', () => {
        ...
        const wkt = 'MULTIPOLYGON(((-96.796 48.756, invalid, -90.379 43.429, -96.796 43.429, -96.796 48.756)))';
        const result = wktToGeoJSON(wkt);
        
        // The function may still return a result with valid coordinates, filtering out invalid ones
        expect(result).toBeDefined();
        expect(consoleSpy).toHaveBeenCalledWith('Invalid coordinate pair:', 'invalid', 'split into:', ['invalid']);
      ...
      it('handles non-numeric coordinates in multipolygon', () => {
        ...
        const wkt = 'MULTIPOLYGON(((-96.796 abc, -90.379 48.756, -90.379 43.429, -96.796 43.429, -96.796 48.756)))';
        const result = wktToGeoJSON(wkt);
        
        // The function may still return a result with valid coordinates, filtering out invalid ones
        expect(result).toBeDefined();
        expect(consoleSpy).toHaveBeenCalledWith('Invalid coordinate values:', ['-96.796', 'abc'], 'from pair:', '-96.796 abc');
  ...
  describe('normalizeGeometry', () => {
    ...
    describe('Error Handling', () => {
      ...
      it('returns null for unknown geometry format', () => {
        ...
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith('Unknown geometry format:', unknownGeometry);
      ...
      it('returns null for invalid WKT string', () => {
        ...
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith('WKT is not a POLYGON or MULTIPOLYGON:', 'INVALID WKT');
    ...
    describe('Edge Cases', () => {
      ...
      it('handles object with empty wkt property', () => {
        ...
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith('WKT is not a POLYGON or MULTIPOLYGON:', '');
