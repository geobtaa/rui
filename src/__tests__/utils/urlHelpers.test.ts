...
describe('urlHelpers', () => {
  ...
  describe('parseFacetUrl', () => {
    ...
    it('parses a valid facet URL with multiple parameters', () => {
      const url = '/catalog?f[dc_publisher_sm][]=MIT%20Libraries&f[gbl_resourceClass_sm][]=Dataset';
      const result = parseFacetUrl(url);
    ...
    it('handles URLs with non-facet parameters', () => {
      const url = '/catalog?q=test&format=json&f[dc_publisher_sm][]=MIT%20Libraries';
      const result = parseFacetUrl(url);
    ...
    it('logs error for invalid URLs', () => {
      ...
      expect(result).toBeNull();
      expect(consoleSpy).toHaveBeenCalledWith('Error parsing facet URL:', expect.any(Error));
      
      consoleSpy.mockRestore();
    });

    it('handles URLs with special characters in field names', () => {
      const url = '/catalog?f[dc_subject_sm][]=Geographic%20Information%20Systems';
      const result = parseFacetUrl(url);
    ...
    it('handles URLs with special characters in values', () => {
      const url = '/catalog?f[dc_publisher_sm][]=MIT%20Libraries%20%26%20Archives';
      const result = parseFacetUrl(url);
  ...
  describe('createFacetSearchUrl', () => {
    it('creates a valid facet search URL with basic parameters', () => {
      ...
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_publisher_sm%5D%5B%5D=MIT+Libraries');
    });

    it('creates URL with resource class facet', () => {
      const result = createFacetSearchUrl('gbl_resourceClass_sm', 'Dataset');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bgbl_resourceClass_sm%5D%5B%5D=Dataset');
    });

    it('creates URL with temporal facet', () => {
      const result = createFacetSearchUrl('dct_temporal_sm', '2020');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdct_temporal_sm%5D%5B%5D=2020');
    });

    it('handles special characters in field names', () => {
      const result = createFacetSearchUrl('dc_subject_sm', 'Geographic Information Systems');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_subject_sm%5D%5B%5D=Geographic+Information+Systems');
    });

    it('handles special characters in values', () => {
      const result = createFacetSearchUrl('dc_publisher_sm', 'MIT Libraries & Archives');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_publisher_sm%5D%5B%5D=MIT+Libraries+%26+Archives');
    });

    it('handles empty field name', () => {
      const result = createFacetSearchUrl('', 'test value');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5B%5D%5B%5D=test+value');
    });

    it('handles empty value', () => {
      const result = createFacetSearchUrl('dc_publisher_sm', '');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_publisher_sm%5D%5B%5D=');
    });

    it('handles numeric values', () => {
      const result = createFacetSearchUrl('dct_temporal_sm', '2023');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdct_temporal_sm%5D%5B%5D=2023');
    });

    it('handles complex field names with underscores', () => {
      const result = createFacetSearchUrl('gbl_resourceClass_sm', 'Point Data');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bgbl_resourceClass_sm%5D%5B%5D=Point+Data');
    });

    it('handles values with spaces and special characters', () => {
      const result = createFacetSearchUrl('dc_publisher_sm', 'University of Minnesota (Twin Cities)');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_publisher_sm%5D%5B%5D=University+of+Minnesota+%28Twin+Cities%29');
    });

    it('creates URL with proper encoding for international characters', () => {
      const result = createFacetSearchUrl('dc_publisher_sm', 'Université de Montréal');
      
      expect(result).toBe('https://geo.btaa.org/?format=json&search_field=all_fields&f%5Bdc_publisher_sm%5D%5B%5D=Universit%C3%A9+de+Montr%C3%A9al');
    });

    it('handles very long field names', () => {
      const longField = 'very_long_field_name_with_many_underscores_and_characters';
    ...
    it('handles very long values', () => {
      const longValue = 'This is a very long value with many words and characters that should be properly encoded in the URL';
      ...
      expect(result).toContain('This+is+a+very+long+value+with+many+words+and+characters+that+should+be+properly+encoded+in+the+URL');
    });
  });
